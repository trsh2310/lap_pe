...
def main(cfg):
    ...
    apply_additional_cfg_params(cfg, val_metrics.get("additional_cfg_params", {}))
...
def apply_additional_cfg_params(cfg, additional_cfg_params):
    if not additional_cfg_params:
        return

    with open_dict(cfg):
        for param_name, param_value in additional_cfg_params.items():
            OmegaConf.update(cfg.model, param_name, param_value, merge=True)

    logger.info(
        "Applied validation-selected model params for test retrain:\n"
        f"{OmegaConf.to_yaml(additional_cfg_params)}\n"
    )
...
def run_train(cfg, verbose: bool = True, test_mode: bool = False):
    ...
    configure_lap_diagnostics(cfg, test_mode=test_mode)
...
def configure_lap_diagnostics(cfg, test_mode: bool):
    if not cfg.model.get("lap_diagnostics_enabled", False):
        return

    run_label = "test_retrain" if test_mode else "validation_train"
    if not cfg.get("_lap_diagnostics_user_output_path_captured", False):
        with open_dict(cfg):
            cfg._lap_diagnostics_user_output_path = cfg.model.get(
                "lap_diagnostics_output_path",
                None,
            )
            cfg._lap_diagnostics_user_output_path_captured = True

    user_output_path = cfg.get("_lap_diagnostics_user_output_path", None)
    with open_dict(cfg.model):
        cfg.model.lap_diagnostics_run_label = run_label
        if user_output_path:
            cfg.model.lap_diagnostics_output_path = user_output_path
        else:
            output_dir = Path(HydraConfig.get().runtime.output_dir)
            cfg.model.lap_diagnostics_output_path = str(
                output_dir / f"lap_diagnostics_{run_label}.jsonl"
            )
