...
def build_laplacian_positional_encoding(train_dataset, k, n_items):
    ...
    eigenvalues, eigvecs = eigsh(L, k=k +1, which='SM')

    idx = np.argsort(eigenvalues)
    eigvecs = eigvecs[:, idx]
    pos_enc = eigvecs[:, 1:1 + k]

    print(f"Using {k} smallest eigenvectors")
...
class SASRecBackBone(nn.Module):
    def __init__(
        ...
        self.lap_proj = nn.Linear(k, hidden_units)
