...
def build_laplacian_positional_encoding(train_dataset, k):
...
class SASRecBackBone(nn.Module):
    ...
    def log2feats(self, log_seqs):
        ...
        item_norm = self.item_emb.weight.norm(dim=1).mean().item()
        lap_norm = lap_pos.norm(dim=1).mean().item()

        scale = item_norm / (lap_norm + 1e-8)
        lap_pos = lap_pos * scale
...
class SASRecEinv(BaseModel):
    ...
    def _post_init(self, train_dataset, val_dataset):
        ...
        lap_pos = build_laplacian_positional_encoding(train_dataset, self.k)
