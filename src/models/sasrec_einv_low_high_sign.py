...
class SASRecBackBone(nn.Module):
    ...
    # --- Laplacian PE ---
        pos_ids = log_seqs
        lap_raw = self.lap_pos_emb(pos_ids)   # (B, T, k)

    # --- Sign-invariant encoding ---
        phi_v = self.sign_phi(lap_raw)
        phi_neg_v = self.sign_phi(-lap_raw)

        lap_part = self.sign_rho(phi_v + phi_neg_v)  # (B, T, hidden)

    # --- debug (редко) ---
        if torch.rand(1).item() < 0.001:
            print("lpe_weight:", self.lpe_weight.item())
            print("item mean norm:", seqs.norm(dim=-1).mean().item())
            print("lap raw mean norm:", lap_raw.norm(dim=-1).mean().item())
            print("lap part mean norm:", lap_part.norm(dim=-1).mean().item())

    # --- combine ---
        seqs = seqs + self.lpe_weight * lap_part
...
class SASRecEinv(BaseModel):
    ...
    # padding для pad_token
        pad_vec = torch.zeros(1, lap_pos.shape[1])
        lap_pos = torch.cat([lap_pos, pad_vec], dim=0)

    # embedding (FIXED)
        self.model.lap_pos_emb = nn.Embedding.from_pretrained(
        lap_pos,
        freeze=True
        ).to(self.device)

    # размер LPE
        self.model.lap_dim = lap_pos.shape[1]

    # --- SIGN-INVARIANT СЕТЬ (SignNet) ---
        self.model.sign_phi = nn.Sequential(
        nn.Linear(self.model.lap_dim, self.hidden_size),
        nn.ReLU(),
        nn.Linear(self.hidden_size, self.hidden_size),
        ).to(self.device)

        self.model.sign_rho = nn.Sequential(
        nn.ReLU(),
        nn.Linear(self.hidden_size, self.hidden_size),
        ).to(self.device)

    # --- оптимизатор (ВАЖНО: после добавления слоёв) ---
