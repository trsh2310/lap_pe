...
def build_laplacian_positional_encoding(train_dataset, k, n_items, seed=None):
    if seed is not None:
        np.random.seed(seed)
    ...
    print(f"Available eigenvectors after removing zero-eigs: {eigvecs.shape[1]}")

    total_available = eigvecs.shape[1]
    if k > total_available:
        raise ValueError(f"k={k} > available eigenvectors={total_available}")

    chosen_idx = np.random.choice(total_available, size=k, replace=False)
    chosen_idx = np.sort(chosen_idx)

    pos_enc = eigvecs[:, chosen_idx]

    return torch.FloatTensor(pos_enc), chosen_idx.tolist()
...
class SASRecEinv(BaseModel):
    ...
    def _post_init(self, train_dataset, val_dataset):
        ...
        lap_pos, chosen_indices = build_laplacian_positional_encoding(train_dataset, self.k, self.n_items, seed=self.lpe_seed)
