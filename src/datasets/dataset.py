...
class RecSysDataset(Dataset):
    def __init__(self, 
        ...
        self._timestamp_index = self._create_timestamp_index()
    ...
    def _create_timestamp_index(self):
        groups = (
            self._df.groupby('user_id')['timestamp']
            .apply(list)
            .to_dict()
        )
        return [
            torch.tensor(groups.get(user_id, []), dtype=torch.long)
            for user_id in self._users
        ]
    ...
    def __getitem__(self, idx):
        result = {
            ...
            "timestamps": self._timestamp_index[idx],
        }
