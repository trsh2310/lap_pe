...
def build_grid_search_space(config: DictConfig) -> dict:
    search_space = {}
    for param in config.optuna_params:
        if param.type != "categorical":
            raise ValueError(
                f"Grid search supports only categorical optuna_params, got {param.type} for {param.name}."
            )
        search_space[param.name] = list(param.choices)
    return search_space
...
class Objective:
    ...
    def __call__(self, trial: Trial) -> float:
        ...
        try:
            ...
            trial.set_user_attr("best_metrics_preview", list(best_metrics.keys())[:20])
            metric_key = f"val/{TARGET_METRIC}"
            if metric_key not in best_metrics:
                trial.set_user_attr("error", f"Missing metric: {metric_key}; keys={list(best_metrics.keys())}")
                return float("-inf")
        except Exception as exc:
            # Сохраняем трассировку в user_attrs и в файл для удобства отладки
            tb = traceback.format_exc()
            trial.set_user_attr("error", repr(exc))
            trial.set_user_attr("traceback", tb)
            err_file = self._tmp_dir / f"trial_{trial.number}_error.txt"
            try:
                err_file.parent.mkdir(parents=True, exist_ok=True)
                err_file.write_text(tb)
                trial.set_user_attr("error_file", str(err_file))
            except Exception:
                pass
            # вернём плохое значение — trial завершён, но не pruned
            return float("-inf")
...
def main(
    ...
    sampler = (
        GridSampler(build_grid_search_space(base_cfg))
        if grid
        else TPESampler(
